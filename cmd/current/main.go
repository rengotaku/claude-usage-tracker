...
func main() {
	...
	if jsonFlag {
		...
		if result.SessionEndsAt != nil {
			out.Session.EndsAt = result.SessionEndsAt.In(timezone.JST).Format("2006-01-02T15:04:05+09:00")
		}
		...
		out.Weekly.ResetsAt = result.WeeklyResetsAt.In(timezone.JST).Format("2006-01-02T15:04:05+09:00")
	...
	resetsAt := result.WeeklyResetsAt.In(timezone.JST).Format("Jan 2, 3pm")
...
func sessionLine(r *service.UsageResult) string {
	...
	if r.SessionEndsAt != nil {
		endsAt = ", resets " + r.SessionEndsAt.In(timezone.JST).Format("3pm (Asia/Tokyo)")
	}
	if r.SessionLimit > 0 {
